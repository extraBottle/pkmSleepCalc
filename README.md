# 포켓몬 슬립 통합 계산기 
**데이터 기반 포켓몬 능력치 측정 서비스**

![Overview of Webpage](./public/images/overview.png)

## 소개
직관적이고 모바일 친화적인 UI로 '누구나 어디서든 가볍게' 포켓몬을 평가할 수는 없을까? 라는 질문에서 시작했습니다.
추가결제 없이 모든 평가 서비스를 사용자 친화적 환경에서 경험해보세요. 현재 아래와 같은 기능을 제공하고 있습니다.
...
2. 구글에서 '포슬립 계산기' 검색하고 '포슬립 계산기 허브' 접속
![google search](./public/images/5google.png)
...
<details>
  <summary>Changelog</summary>
    ...
    **v1.4.1**
    - 포켓몬 암팰리스, 보만다, 플라이곤, 이브이(할로윈) 추가
    - 식재료 반들반들아보카도 추가
    - crit chance 유무에 따른 ingredient draw S 스킬 구분

    **v1.4.2**
    - favicon 교체

    **v1.4.3**
    - Z fold 화면에서 ui 오류 수정

    **v1.5.0**
    - 식재료 계산기 자힐 계산 공식 (확률 기댓값 -> 회복량 기댓값)으로 변경
    - 식재료 계산기 '추가 힐러 없음'으로 텍스트 변경

    **v1.5.1**
    - energizing cheer 스킬 기댓값 계산 공식 변경
